...
export function CameraDetection() {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    console.log('完整URL:', `${wsUrl}?token=${encodedToken}`)
    ...
    ws.onerror = (event) => {
      ...
      console.error('错误事件类型:', event.type)
        ...
        await new Promise((resolve) => {
          if (videoRef.current.videoWidth > 0) {
            ...
            videoRef.current.onloadedmetadata = () => {
              console.log('视频元数据加载完成:', videoRef.current.videoWidth, videoRef.current.videoHeight)
              resolve()
            }
          }
        })
  ...
  const sendFrame = useCallback(() => {
    ...
    if (video.videoWidth === 0 || video.videoHeight === 0) {
      console.log('视频尺寸为0，跳过发送帧:', video.videoWidth, video.videoHeight)
      return
    }

    console.log('发送帧:', video.videoWidth, video.videoHeight)
    ...
    console.log('帧数据大小:', base64Data?.length, 'bytes')
      ...
      <div className="flex-1 flex flex-col lg:flex-row p-6 gap-6 overflow-hidden">
        <div className={`flex-1 relative rounded-xl overflow-hidden border ${isLight ? 'bg-white border-gray-200' : 'bg-dark-800 border-dark-700'}`}>
          <div className="w-full h-full relative">
            <video
              ref={videoRef}
              className={`w-full h-full object-cover ${!cameraAccess ? 'hidden' : ''}`}
              autoPlay
              playsInline
              muted
            />
            
            {isDetecting && annotatedFrame && (
              <img
            ...
            {!cameraAccess && (
              <div className={`absolute inset-0 flex flex-col items-center justify-center ${isLight ? 'text-gray-400' : 'text-dark-500'}`}>
