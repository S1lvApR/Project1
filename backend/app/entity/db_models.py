...
数据库模型定义
表结构总览：
    ⽤户权限：users, roles, permissions, user_roles, role_permissions
...
class User(Base):
    ...
    username = Column(
        String(50), unique=True, nullable=False, index=True, comment="⽤户名"
    )
...
class Role(Base):
    ...
    name = Column(
        ...
        unique=True,
        nullable=False,
        comment="⻆⾊标识，如admin/operator/viewer",
    )
    display_name = Column(
        String(100), nullable=False, comment="⻆⾊显示名，如 管理员/操作员/访客"
    )
    description = Column(String(500), nullable=True, comment="⻆⾊描述")
    is_system = Column(Boolean, default=False, comment="是否系统内置⻆⾊（不可删除）")
    created_at = Column(DateTime, default=datetime.now, comment="创建时间")
    # 关联
    user_roles = relationship(
...
class DetectionScene(Base):
    """检测场景配置表
    每个⼩组/业务⽅向⼀个场景，如：遥感检测、⼯业缺陷、农业病害等
    场景决定了使⽤哪个模型、检测哪些类别
    """
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    name = Column(
        String(100), unique=True, nullable=False, comment="场景标识，如remote_sensing"
    )
    display_name = Column(
        String(100), nullable=False, comment="场景显示名，如 遥感⽬标检测"
    )
    ...
    class_names_cn = Column(
        JSON, nullable=True, comment='类别中⽂名映射，如{"airplane":"⻜机"}'
    )
    is_active = Column(Boolean, default=True, comment="是否启⽤")
    created_by = Column(
        Integer, ForeignKey("users.id"), nullable=True, comment="创建⼈"
    )
    created_at = Column(DateTime, default=datetime.now)
...
class DetectionTask(Base):
    """检测任务表 — 每次检测操作⽣成⼀条任务记录"""

    __tablename__ = "detection_tasks"
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, index=True, comment="操作⽤户"
    )
    scene_id = Column(
        Integer,
    ...
    total_objects = Column(Integer, default=0, comment="检测到⽬标总数")
    ...
    image_size = Column(Integer, default=640, comment="推理图像尺⼨")
    # 错误信息
    ...
    risk_level = Column(
        String(20), nullable=True, comment="⻛险等级：low/medium/high/critical"
    )
...
class DetectionResult(Base):
    ...
    class_name_cn = Column(String(50), nullable=True, comment="类别中⽂名")
    ...
    image_height = Column(Integer, nullable=True, comment="图像⾼度")
    created_at = Column(DateTime, default=datetime.now)
    # 关联
...
class TrainingTask(Base):
    ...
    task_uuid = Column(
        String(100), unique=True, nullable=False, index=True, comment="任务唯⼀标识"
    )
    status = Column(
        String(20),
        default="pending",
    ...
    img_size = Column(Integer, default=640, comment="图像尺⼨")
    batch_size = Column(Integer, default=16, comment="批次⼤⼩")
    ...
    progress = Column(Integer, default=0, comment="进度百分⽐ 0~100")
    # 数据集信息
...
class ModelVersion(Base):
    ...
    # 模型⽂件
    model_path = Column(String(500), nullable=False, comment="本地模型⽂件路径")
    ...
    file_size = Column(BigInteger, nullable=True, comment="模型⽂件⼤⼩（字节）")
...
class ChatSession(Base):
    ...
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(
        Integer, ForeignKey("users.id"), nullable=False, index=True, comment="所属⽤户"
    )
    session_uuid = Column(
        String(100), unique=True, nullable=False, index=True, comment="会话唯⼀标识"
    )
    title = Column(String(200), nullable=True, comment="会话标题（取第⼀条消息摘要）")
...
class ChatMessage(Base):
    ...
    tool_result = Column(Text, nullable=True, comment="⼯具调⽤返回结果")
    # 元信息
...
class OperationLog(Base):
    ...
    username = Column(String(50), nullable=True, comment="冗余⽤户名，⽅便查询")
    # 操作信息
    module = Column(
        String(50),
        nullable=False,
    ...
    request_method = Column(String(10), nullable=True, comment="HTTP ⽅法")
...
class FileCache(Base):
    ...
    username = Column(String(50), nullable=False, comment="上传用户名")
    uploaded_at = Column(DateTime, default=datetime.now, index=True, comment="上传时间")
