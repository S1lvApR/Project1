...
class AIService:
    ...
    def __init__(self):
        self.provider = settings.AI_PROVIDER.lower() if hasattr(settings, 'AI_PROVIDER') else "openai"
...
- 如果用户的问题与交通标志、信号灯识别无关，你必须回答：无法做到。
...
- 不要回答天气、新闻、娱乐、编程等无关话题。"""
    ...
    def _create_client(self):
        ...
        if self.provider == "openai":
            ...
            base_url = getattr(settings, 'BAILIAN_API_BASE', 'https://dashscope.aliyuncs.com/compatible-mode/v1')
            return httpx.AsyncClient(
                base_url=base_url,
                headers={"Authorization": f"Bearer {api_key}"},
                timeout=timeout
    ...
    async def chat_with_tools(self, messages: List[Dict], model: Optional[str] = None) -> str:
        try:
            system_prompt = self._get_system_prompt()
    ...
    async def _call_with_tools(self, messages: List[Dict], model: Optional[str] = None) -> Dict:
    ...
    async def _call_openai(self, messages: List[Dict], model: Optional[str] = None) -> str:
        try:
    ...
    async def _call_baidu(self, messages: List[Dict], model: Optional[str] = None) -> str:
        try:
    ...
    async def _call_doubao(self, messages: List[Dict], model: Optional[str] = None) -> str:
        try:
    ...
    async def _call_qianwen(self, messages: List[Dict], model: Optional[str] = None) -> str:
        try:
            req_model = model or getattr(settings, 'QIANWEN_MODEL', 'qwen2-72b-instruct')
            response = await self.client.post(
                "/chat/completions",
                json={
                    "model": req_model,
    ...
    async def _call_bailian(self, messages: List[Dict], model: Optional[str] = None) -> str:
        try:
            ...
            if response.status_code == 403:
                ...
                raise Exception(f"API Key 无效或无权限访问模型。请检查 .env 中的 BAILIAN_API_KEY 是否正确。错误详情: {error_detail}")
