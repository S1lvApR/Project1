...
function Resolve-PythonRuntime {
    ...
    if ($null -ne $pathPython) {
        try {
            $actualVersion = Get-ExactPythonVersion -PythonPath $pathPython
            if ($actualVersion -eq $expectedVersion) {
                return $pathPython
            }

            throw "Python version mismatch. Expected $expectedVersion but found $actualVersion at '$pathPython'."
        } catch {
            throw
        }
    }

    throw "Could not find Python $expectedVersion on PATH. Please install Python $expectedVersion and ensure it is available on your system PATH."
}
