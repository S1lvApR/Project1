...
export const useStore = create((set, get) => ({
  ...
  ensureConversationPersisted: async (conversationId) => {
    const token = getToken()
    if (!token) {
      console.warn('No token found, returning original conversationId:', conversationId)
    ...
    if (conv.persisted) {
      console.log('Conversation already persisted:', conversationId)
      return conversationId
    }
    try {
      console.log('Persisting conversation:', conversationId)
      const data = await request(`/chat-sessions?title=${encodeURIComponent(conv.title || 'New Chat')}`, {
        method: 'POST',
      })
      if (data.success && data.data) {
        const newId = data.data.id
        console.log('Conversation persisted successfully, new ID:', newId)
        set((state) => ({
          conversations: state.conversations.map(c =>
            c.id === conversationId
              ? {
  ...
  sendMessage: async (conversationId, content) => {
    console.log('sendMessage called with conversationId:', conversationId, 'content:', content)
    conversationId = await get().ensureConversationPersisted(conversationId)
    console.log('After ensureConversationPersisted, conversationId:', conversationId)
    ...
    try {
      ...
      const uploadCardMessageId = (Date.now() + 2).toString()
      ...
      const token = getToken()
      console.log('Fetching chat/message with session_uuid:', conversationId, 'token:', token ? 'exists' : 'null')
          ...
          if (line.startsWith('data: ')) {
            try {
              ...
              if (eventData.type === 'done') {
                await get().saveMessage(conversationId, 'assistant', fullText)

                if (fullText.trim() === '好的，请在下方出现的卡片中上传您需要识别的图片、文件夹或ZIP压缩包。') {
                  set((state) => ({
                    conversations: state.conversations.map((c) =>
                      c.id === conversationId
                        ? { ...c, messages: [...c.messages, {
                            id: uploadCardMessageId,
                            conversationId,
                            role: 'assistant',
                            content: '',
                            createdAt: new Date(),
