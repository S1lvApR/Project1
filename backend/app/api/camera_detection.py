...
async def camera_detection_websocket(websocket: WebSocket):
    logger.info(f"WebSocket连接尝试 - Origin: {websocket.headers.get('origin')}")
    logger.info(f"WebSocket连接尝试 - Path: {websocket.url.path}")
    logger.info(f"WebSocket连接尝试 - Query: {websocket.query_params}")
